...
**Box Plot** **I THINK WE CAN ADD THIS** 
The boxplot illustrates the response variable across five weeks, separated by inoculation method (Dry, Wet) and thickness (1/4-inch, 1/8-inch).
Differences in medians between Dry and Wet methods suggest potential interactions, particularly in certain weeks.
The response patterns differ between 1/4-inch and 1/8-inch thickness, with 1/4-inch showing more consistency over time.
Variability in responses increases in certain weeks, such as Week 5 for 1/4-inch and Wet, indicating possible heterogeneity in variance.
Overall, the plot highlights the importance of analyzing the interaction effects and temporal correlations in the repeated measures model.
...
**WHAT DO YOU THINK ABOUT THIS PLOT?** Based on lsmean.

If you want we can add two other interaction plots as well?

The interaction plot suggests that Thickness has a differential effect on the response over time, with 1/8-inch showing greater variability and sharper trends compared to 1/4-inch.
This interaction warrants further statistical confirmation through hypothesis testing.
...
data <- data.frame(
  Thickness = rep(c("1/4-inch", "1/8-inch"), each = 5),
  Week = rep(1:5, times = 2),
  Estimate = c(4.2195, 4.3300, 4.3801, 4.3315, 4.3721, 
               4.7491, 4.8049, 4.5628, 4.3238, 4.5135),
  SE = rep(0.1013, 10) # Standard Error
)

# Interaction plot with error bars
ggplot(data, aes(x = Week, y = Estimate, color = Thickness, group = Thickness)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = Estimate - SE, ymax = Estimate + SE), width = 0.2) +
  labs(title = "Interaction Plot: Thickness vs Week",
       x = "Week",
       y = "Least Squares Means (Estimate)") +
  theme_minimal() +
...
While both the inoculation method and thickness are significant, this is just the main effects for the model.
Therefore, we need to focus on the interaction terms.

All interaction contrasts are significant, with Dry vs. Wet at 1/8 inches and 1/4 vs. 1/8 inches for Dry being highly significant.
This highlights that the bacteria seems to grow higher when dry inoculated at the two thicknesses levels (especially on 1/8 inches)
...
## Are there any tables we don't need- I think we need all those to explain properly. As we already showed all those in presentation, we can add in report
...
The interaction between inoculation method and time (Week) is evident, with Dry inoculation showing an increase in Week 5 and Wet inoculation peaking earlier.
This indicates that the timing of bacterial growth depends on the inoculation method used.

![fig6](ar-6.png){#fig-iXW width="70%"}

The interaction between Inoculation_Method and Week is significant, with differences in bacterial growth patterns being particularly pronounced for Week 5, where the Dry method shows significantly higher responses than Wet.
This suggests the impact of inoculation method depends on the week of measurement.
